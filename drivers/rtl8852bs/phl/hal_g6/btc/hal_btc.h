...
enum btc_fw_btc_error {
	...
	BTC_FWERR_AUTO_EXCEPTION = BIT(7),
...
struct btc_ant_info {
	...
	u8 single_pos: 1;/* wifi 1ss-1ant at 0:S0 or 1:S1 */
	u8 diversity: 1; /* only for wifi use 1-antenna */
	u8 btg_pos: 2; /* btg-circuit at 0:S0/1:S1/others:all */
	u8 stream_cnt: 4;  /* spatial_stream count */
};
...
struct btc_module {
	...
	u8 wa_type: 3; /* WA type: 0:none, 1: 51B 5G_Hi-Ch_Rx */
...
void _send_fw_cmd(struct btc_t *btc, u8 h2c_func, u8 *param, u16 len);
