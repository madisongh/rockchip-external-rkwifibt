...
struct fbtc_slot s_def[] = {
	...
	{  0, 0xea555555, SLOT_MIX}, /* CXST_EBT */
...
void _set_bt_tx_power(struct btc_t *btc, u32 level)
{
	...
	buf = level & bMASKB0;
	_send_fw_cmd(btc, SET_BT_TX_PWR, &buf, 1);
}
...
void _set_bt_rx_gain(struct btc_t *btc, u32 level)
{
	...
	buf = level & bMASKB0;
	_send_fw_cmd(btc, SET_BT_LNA_CONSTRAIN, &buf, 1);
...
static void _set_bt_golden_rx_range(struct btc_t *btc, u8 p_id, u8 level)
{
	...
	buf[0] = level;
	buf[1] = pos;
	_send_fw_cmd(btc, SET_BT_GOLDEN_RX_RANGE, buf, 2);
}
...
static void _set_bt_afh_info(struct btc_t *btc)
{
	...
	wl_afh->ch = ch;
	wl_afh->bw = bw;
	_send_fw_cmd(btc, SET_BT_WL_CH_INFO, (u8*)wl_afh, 3);
...
static void _set_phl_bt_slot_req(struct btc_t *btc)
{
	...
	if (!run_rsn("_ntfy_wl_rfk") && wl->rfk_info.state == BTC_WRFK_STOP)
		btc->bt_req_en = false;
...
void _action_wl_2g_go(struct btc_t *btc)
{
	...
	if (btc->mdinfo.ant.type == BTC_ANT_SHARED) { /* shared-antenna */
		if (wl->status.map._4way)
			...
			policy_type = BTC_CXP_OFFE_2GBWISOB;
		else if (dm->client_ps_tdma_on == 1)
			policy_type = BTC_CXP_OFF_BWB0;
		else if (bt->link_info.status.map.connect == 0)
			policy_type = BTC_CXP_OFFE_2GISOB;
		else
