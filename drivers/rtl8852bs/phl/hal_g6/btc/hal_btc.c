...
static void _ntfy_specific_packet(struct btc_t *btc, u8 pkt_type)
{
	...
	case PKT_EVT_EAPOL_START:
		...
		/* To vaoid no EAPOL_END and lead to _4way always = 1  */
		_set_btc_timer(btc, BTC_TIMER_WL_SPECPKT, BTC_SPKT_MAXT);
		break;
...
static void _ntfy_radio_state(struct btc_t *btc, u8 rf_state)
{
	...
	if (wl_smap->lps_pre == BTC_LPS_OFF && wl_smap->lps_pre != wl_smap->lps)
		btc->dm.tdma_instant_excute = 1;
			 ...
			 u8 reason)
{
	...
	if (status_change) {
		...
		if (wl_smap->busy && wl->rssi_level <= 2 &&
		    wl->is_5g_hi_channel && btc->mdinfo.wa_type == 1)
...
#else
	u32 path = (type & bMASKB1) >> 8;
	u32 reg_type = type & bMASKB0;
	u32 ori_val, i;
	u32 shift = 0;
